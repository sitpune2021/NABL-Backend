...
class Zone extends Model
{
    ...
    {
        return $this->hasMany(Cluster::class, 'zone_id');
    }

    // 🔗 Lab override → Master zone
    public function parent()
    {
        return $this->belongsTo(Zone::class, 'parent_id');
    }

    // 🔁 Master zone → Lab overrides
    ...
    public function lab()
    {
        return $this->hasOneThrough(
            ...
            Zone::class, // Intermediate (Lab Zone)

            'id',            // Intermediate PK (zones.id)
            'id',            // Final PK (labs.id)

            'parent_id',     // FK on SuperAdmin Zone
            'owner_id'       // FK on Lab Zone
        )->where('zones.owner_type', 'lab')  // 🔥 VERY IMPORTANT
    ...
    {
        return $this->hasOne(Zone::class, 'parent_id')
